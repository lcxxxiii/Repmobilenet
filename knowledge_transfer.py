...
from models.model_repvgg import create_RepVGG_A2 as model_t
from models.model_repmobilenet import Repmobilenet as model_s

from utils import save_checkpoint, cal_para
from models.distillation import cosine_similarity, scale_process
...
parser = argparse.ArgumentParser(description='Multi-layer knowledge transfer')
...
def train(train_list, teacher, student, criterion, optimizer, epoch):
    ...
    for i, (img, target) in enumerate(train_loader):
        ...
        with torch.no_grad(): #教师网络不参与反向传播，不更新权重
        ...
        loss_h = criterion(student_output, target) #求学生网络输出和gt的loss
        loss_s = criterion(student_output, teacher_output) #求学生网络输出和教师网络输出的loss
